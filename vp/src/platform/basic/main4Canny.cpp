// src/platform/basic/main4Canny.cpp: hack for EECS 226 (05/21/21, RD)
// -> replaced "sensor2" with "camera"
// Adapted by manfred.schlaegl@jku.at (01/17/22, 12/12/23)
...
class BasicOptions : public Options {
   public:
	...
	void parse(int argc, char **argv) override {
		...
		assert((mem_end_addr < clint_start_addr || mem_start_addr > flash_end_addr) &&
...
int sc_main(int argc, char **argv) {
	...
	// address mapping
	{
		...
		bus.ports[it++] = new PortMapping(opt.camera_start_addr, opt.camera_end_addr);
